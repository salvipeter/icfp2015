...
(defun unit-width (unit)
  (let ((xs (mapcar #'first (members unit))))
    (- (reduce #'max xs) (reduce #'min xs))))

(let ((go-east t) best-width)
  (defun solve-current (&optional initp)
    (when initp
      (setf best-width (unit-width *current*)))
    (when *ps-stream*
      (write-ps-board *ps-stream*))
    (let ((rc (rotate-unit-clockwise *current*))
          (rcc (rotate-unit-counter-clockwise *current*))
          (e (if go-east (move-unit-e *current*) (move-unit-w *current*)))
          (sw (if go-east (move-unit-sw *current*) (move-unit-se *current*)))
          (se (if go-east (move-unit-se *current*) (move-unit-sw *current*))))
      (cond ((and (< (unit-width rc) best-width) (placeablep rc))
             (setf best-width (unit-width rc))
             (remove-current)
             (setf *current* rc)
             (place-current)
             (cons 'rc (solve-current)))
            ((and (< (unit-width rcc) best-width) (placeablep rcc))
             (setf best-width (unit-width rcc))
             (remove-current)
             (setf *current* rcc)
             (place-current)
             (cons 'rcc (solve-current)))
            ((placeablep e)
             (setf best-width (min best-width (unit-width e)))
             (remove-current)
             (setf *current* e)
             (place-current)
             (cons (if go-east 'e 'w) (solve-current)))
            ((placeablep sw)
             (setf best-width (min best-width (unit-width sw)))
             (remove-current)
             (setf *current* sw)
             (place-current)
             (cons (if go-east 'sw 'se) (solve-current)))
            ((placeablep se)
             (setf best-width (min best-width (unit-width se)))
             (remove-current)
             (setf *current* se)
             (place-current)
             (cons (if go-east 'se 'sw) (solve-current)))
            (t (lock-current)
               (when (< (reduce #'min (mapcar #'second (members *current*))) 3)
                 (setf go-east (not go-east)))
               '(sw))))))
