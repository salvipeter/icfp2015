...
(defun move-unit-dir (unit dir)
  (ecase dir
    (w (move-unit-w unit))
    (e (move-unit-e unit))
    (sw (move-unit-sw unit))
    (se (move-unit-se unit))))
...
(let ((go-east t) best-width last)
  (defun solve-current (&optional initp)
    ...
    (flet ((try-movement (dir)
             (let ((u (move-unit-dir *current* dir)))
               (when (placeablep u)
                 (setf best-width (min best-width (unit-width u)))
                 (when *ps-stream*
                   (remove-current))
                 (setf *current* u)
                 (when *ps-stream*
                   (place-current))
                 dir)))
           (try-rotation (clockwisep)
             (let* ((u (rotate-unit *current* clockwisep))
                    (w (unit-width u)))
               (when (and (< w best-width) (placeablep u))
                 (setf best-width w)
                 (when *ps-stream*
                   (remove-current))
                 (setf *current* u)
      ...
      (let ((next (or (and (not (eq last (if go-east 'w 'e)))
                           (try-movement (if go-east 'e 'w)))
                      (let ((down-order (shuffle '(se sw))))
                        (or (try-movement (first down-order))
                            (try-movement (second down-order)))) 
                      (and (not (eq last (if go-east 'e 'w)))
                           (try-movement (if go-east 'w 'e)))
                      (let ((rot-order (shuffle '(t nil))))
...
(defun main ()
  (let* ((n (length sb-ext:*posix-argv*))
         (files (iter (for i from 1 below n by 2)
    ...
    (if (null files)
        ...
        (cl-json:encode-json (coerce (iter (for file in files) (appending (run-file file))) 'vector)
                             *standard-output*))))
